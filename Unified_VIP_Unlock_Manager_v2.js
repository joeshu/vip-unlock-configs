...
const LockManager = {
  ...
  acquire(url) {
    const key = `__vip_lock_${Utils?.simpleHash ? Utils.simpleHash(url || Date.now().toString()) : (url || Date.now())}`;
    const now = Date.now();
    ...
    for (const [k, v] of this.locks) {
      if (now - v.time > 30000) {
        ...
        this.locks.delete(k);
        Logger?.debug && Logger.debug('Lock', `Auto-released expired lock: ${k}`);
      }
    }
    
    // 检查重复执行
    if (this.locks.has(key)) {
      Logger?.debug && Logger.debug('Lock', `Duplicate execution detected: ${url?.substring(0, 50)}`);
    ...
    Logger?.debug && Logger.debug('Lock', `Acquired: ${key}`);
  ...
  release(key) {
    ...
    Logger?.debug && Logger.debug('Lock', `Released: ${key}`);
  }
};
...
const GlobalRegistry = {
  ...
  clear() {
    ...
    this.keys.clear();
    Logger?.debug && Logger.debug('Registry', `Cleared ${count} global variables`);
  }
};
