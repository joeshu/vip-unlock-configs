...
[rewrite_local]
 ...
 ^https?:\/\/(?:api[23]\.tophub\.(?:xyz|today|app)|tophub(?:2)?\.(?:tophubdata\.com|idaily\.today|remai\.today|iappdaiy\.com|ipadown\.com))\/account\/sync url script-response-body https://raw.githubusercontent.com/joeshu/vip-unlock-configs/refs/heads/main/Unified_VIP_Unlock_Manager_v5.js
 # gps - GPS工具箱（JSON模式-声明式字段设置）
 ^https:\/\/service\.gpstool\.com\/app\/index\/getUserInfo url script-response-body https://raw.githubusercontent.com/joeshu/vip-unlock-configs/refs/heads/main/Unified_VIP_Unlock_Manager_v5.js
 ^https?:\/\/mapi\.kouyuxingqiu\.com\/api\/v2 url script-response-body https://raw.githubusercontent.com/joeshu/vip-unlock-configs/refs/heads/main/Unified_VIP_Unlock_Manager_v5.js
 # mhlz - 魔幻粒子（JSON模式-声明式前缀处理）
 ^https?:\/\/ss\.landintheair\.com\/storage\/ url script-response-body https://raw.githubusercontent.com/joeshu/vip-unlock-configs/refs/heads/main/Unified_VIP_Unlock_Manager_v5.js
 # v2ex - V2EX去广告（HTML替换模式）
 ^https?:\/\/.*v2ex\.com\/(?!(?:.*(?:api|login|cdn-cgi|verify|auth|captch|\.(js|css|jpg|jpeg|png|webp|gif|zip|woff|woff2|m3u8|mp4|mov|m4v|avi|mkv|flv|rmvb|wmv|rm|asf|asx|mp3|json|ico|otf|ttf)))).+$ url script-response-body https://raw.githubusercontent.com/joeshu/vip-unlock-configs/refs/heads/main/Unified_VIP_Unlock_Manager_v5.js
 # foday - 复游会去广告（多路径模式-声明式过滤）
 ^https?:\/\/apis\.folidaymall\.com\/online\/capi\/component\/getPageComponents url script-response-body https://raw.githubusercontent.com/joeshu/vip-unlock-configs/refs/heads/main/Unified_VIP_Unlock_Manager_v5.js
 # qiujingapp - 球竞APP去广告（多路径模式-声明式清空）
 ^https?:\/\/gateway-api\.yizhilive\.com\/api\/v2\/index\/carouses\/(3|6|8|11)(\?.*)?$ url script-response-body https://raw.githubusercontent.com/joeshu/vip-unlock-configs/refs/heads/main/Unified_VIP_Unlock_Manager_v5.js
 ^https?:\/\/gateway-api\.yizhilive\.com\/api\/v3\/index\/all\?.*position=2.*$ url script-response-body https://raw.githubusercontent.com/joeshu/vip-unlock-configs/refs/heads/main/Unified_VIP_Unlock_Manager_v5.js
 # Keep - 课程/会员接口（正则替换模式）
 ^https?:\/\/(api|kit)\.gotokeep\.com\/(nuocha|gerudo|athena|nuocha\/plans|suit\/v5\/smart|kprime\/v4\/suit\/sales)\/ url script-response-body https://raw.githubusercontent.com/joeshu/vip-unlock-configs/refs/heads/main/Unified_VIP_Unlock_Manager_v5.js
 # bqwz - 标枪王者游戏数据接口（游戏数值模式）
 ^https?:\/\/javelin\.mandrillvr\.com\/api\/data\/get_game_data url script-response-body https://raw.githubusercontent.com/joeshu/vip-unlock-configs/refs/heads/main/Unified_VIP_Unlock_Manager_v5.js
 # bxkt - 伴学课堂接口（混合模式-声明式组合）
 ^https?:\/\/api\.banxueketang\.com\/api\/classpal\/app\/v1 url script-response-body https://raw.githubusercontent.com/joeshu/vip-unlock-configs/refs/heads/main/Unified_VIP_Unlock_Manager_v5.js
 # tv - 影视去广告接口（多路径模式-完全声明式）
 ^https?:\/\/(yzy0916|yz1018|yz250907|yz0320|cfvip)\..+\.com\/(v2|v1)\/api\/(basic\/init|home\/firstScreen|adInfo\/getPageAd|home\/body) url script-response-body https://raw.githubusercontent.com/joeshu/vip-unlock-configs/refs/heads/main/Unified_VIP_Unlock_Manager_v5.js
 ...
 ^https?:\/\/yr-game-api\.feigo\.fun\/api\/user\/get-game-user-value url script-response-body https://raw.githubusercontent.com/joeshu/vip-unlock-configs/refs/heads/main/Unified_VIP_Unlock_Manager_v5.js
 # 星际使命 - 微信小程序游戏数据修改（JSON声明式处理器-完全重构）
 ^https?:\/\/star\.jvplay\.cn\/v2\/storage url script-response-body https://raw.githubusercontent.com/joeshu/vip-unlock-configs/refs/heads/main/Unified_VIP_Unlock_Manager_v5.js
# wohome - 联通智家去广告（条件删除模式）
 ^https:\/\/iotpservice\.smartont\.net\/wohome\/dispatcher url script-response-body https://raw.githubusercontent.com/joeshu/vip-unlock-configs/refs/heads/main/Unified_VIP_Unlock_Manager_v5.js
# 思朗月影视 - 用户信息VIP解锁
^https?:\/\/theater-api\.sylangyue\.xyz\/api\/user\/info url script-response-body https://raw.githubusercontent.com/joeshu/vip-unlock-configs/refs/heads/main/Unified_VIP_Unlock_Manager_v5.js
 [mitm]
...
// ==========================================
// P0-1: 正则编译缓存系统（关键修复：转义保护）
// ==========================================
const RegexCache = {
  _key: 'vip_regex_cache_v4_fixed', // 更新缓存键，强制刷新旧缓存
  
  // 关键修复：保护转义字符，避免双重转义
  _escapeRegexSource(source) {
    // 将 source 中的 \ 替换为 \\，确保 JSON 序列化后不会丢失
    // 注意：这是为了存储，不是改变正则语义
    return source.replace(/\\/g, '\\\\');
  },
  
  // 关键修复：恢复转义字符
  _unescapeRegexSource(escapedSource) {
    // 将存储的 \\ 恢复为 \
    return escapedSource.replace(/\\\\/g, '\\');
  },
  
  // 生成缓存键（基于 manifest 内容哈希）
  ...
  load(manifest) {
    try {
      ...
      if (!stored) {
        Logger.debug('RegexCache', 'No stored cache found');
        return null;
      }
      
      const data = Utils.safeJsonParse(stored);
      if (!data || data.cacheKey !== cacheKey) {
        Logger.debug('RegexCache', `Cache key mismatch: expected ${cacheKey}, got ${data?.cacheKey}`);
        return null;
      }
      
      // 检查时间戳
      if (Date.now() - data.timestamp > CONFIG.CACHE_TTL) {
        Logger.debug('RegexCache', 'Cache expired');
        return null;
      }
      ...
      for (const [id, regexData] of Object.entries(data.patterns || {})) {
        try {
          // 关键：先恢复转义，再创建 RegExp
          const originalSource = this._unescapeRegexSource(regexData.source);
          patterns.set(id, new RegExp(originalSource, regexData.flags));
          Logger.debug('RegexCache', `Restored regex ${id}: /${originalSource}/${regexData.flags}`);
        } catch (e) {
          Logger.debug('RegexCache', `Failed to restore regex ${id}: ${e.message}`);
        }
      }
  ...
  save(patterns, domainIndex, manifest) {
    try {
      ...
      for (const [id, regex] of patterns) {
        // 关键：保存转义后的 source，避免 JSON 序列化丢失 \
        const protectedSource = this._escapeRegexSource(regex.source);
        serializablePatterns[id] = {
          source: protectedSource,
          flags: regex.flags
        };
        Logger.debug('RegexCache', `Saving regex ${id}: /${protectedSource}/${regex.flags} (original: /${regex.source}/${regex.flags})`);
      }
...
class RuntimeLoader {
 ...
 // P0-1: 尝试从缓存加载正则（传入 null，load 内部处理）
 const cachedRegex = RegexCache.load(null);
